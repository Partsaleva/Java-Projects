...
public class DronesControl {
	...
	private int returnMinutes(DTimestamp t){
		return t.getDate().getDd()*24*60 + t.getTime().getHh() * 60 + t.getTime().getMm();
	...
	public void chargeUsedDrones(Warehouse warehouse,List<Drone> dronesForDelivery,int distance, DTimestamp dTimestamp) {
		...
		charge.add(new ChargingStation(dronesForDelivery, distance*2, dTimestamp));
		
		for (ChargingStation chStat : charge) {
			//calculate minutes between current time and saved for drones before
			int minutes = returnMinutes(dTimestamp) - returnMinutes(chStat.getTimestamp());
